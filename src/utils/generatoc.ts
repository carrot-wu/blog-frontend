...
interface Generatoc {
  init: ({content, heading, selector, scrollElement}: Params) => void;
  destroy: () => void;
  refresh: () => void;
}
...
let scrollEle: Element | Window = window
...
function scrollEaseOut(start: number, destination: number = 0, rate: number = 2): void {
  ...
  scrollEle.scrollTo(0, destination)
  function jump(): void {
    ...
    scrollEle.scrollTo(0, currentPosition);
...
function handlePageChange() {
  ...
  window.requestAnimationFrame(function () {
    ...
    elem = (tocA as HTMLElement).closest('ul')
    if (elem) {
      triggerShow(elem)
    } else {
      return
...
function clickEvent(e: Event) {
  ...
  let scrollTop = 0
  if (scrollEle instanceof Window) {
    scrollTop = document.documentElement.scrollTop || document.body.scrollTop
  } else {
    scrollTop = scrollEle.scrollTop
  }
  const destination = elementOffset(headingNode[+index!]).top - tocScrollOffset
  // 这边增加了scrollTop就没事了 具体还不清楚什么原因= =
  scrollEaseOut(scrollTop, scrollTop + destination, tocDuration)
}
...
function showEvent(e: Event) {
  e.stopPropagation()
  triggerShow(e.target as HTMLElement)
}
...
function renderToc() {
  ...
  if (headingNode.length > 0) {
    scrollEle.addEventListener("scroll", throttle(handlePageChange), false);
  }
}
...
const generatoc: Generatoc = {
  init({content, heading = ['h2', 'h3', 'h4', 'h5'], selector = '#toc', scrollHistory = false, scrollOffset = 0, duration = 7, scrollElement}: Params) {
    ...
    if (scrollElement) {
      const queryScrollEle = document.querySelector(scrollElement)
      scrollEle = queryScrollEle || scrollEle
    }
